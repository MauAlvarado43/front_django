...
docker-compose -f bin/docker/docker-compose-dev.yml run front_django /bin/sh -c "chmod +x bin/docker/*.sh"

echo == Creating .env.devs
docker-compose -f bin/docker/docker-compose-dev.yml run front_django /bin/sh -c "cp bin/docker/env-dev.sh bin/docker/win-env-dev.sh"
docker-compose -f bin/docker/docker-compose-dev.yml run front_django /bin/sh -c "sed -i 's/\r$//g' bin/docker/win-env-dev.sh"
docker-compose -f bin/docker/docker-compose-dev.yml run front_django /bin/sh -c "bin/docker/win-env-dev.sh %DJANGO_PORT% %POSTGRES_PORT% %REDIS_PORT% %SERVER_URL% %CLIENT_URL%"
...
docker-compose -f bin/docker/docker-compose-dev.yml exec front_django /bin/sh -c "python manage.py loaddata bin/docker/fixtures-dev.yaml"

echo == Generating docs
docker-compose -f bin/docker/docker-compose-dev.yml exec front_django /bin/sh -c "sphinx-build -E -b html ./docs ./.data/docs"
...
echo == Cleaning setup
docker-compose -f bin/docker/docker-compose-dev.yml exec front_django /bin/sh -c "rm bin/docker/win-env-dev.sh"
