...
sudo docker-compose -f bin/docker/docker-compose-dev.yml run front_django /bin/sh -c "chmod +x bin/*.sh;chmod +x bin/docker/*.sh"

echo "== Creating .env.devs"
sudo docker-compose -f bin/docker/docker-compose-dev.yml run front_django /bin/sh -c  "bin/docker/env-dev.sh $DJANGO_PORT $POSTGRES_PORT $REDIS_PORT $SERVER_URL $CLIENT_URL"

echo "== Starting services"
sudo docker-compose -f bin/docker/docker-compose-dev.yml up -d

echo "== Executing db update (make & run migrations)"
sudo docker-compose -f bin/docker/docker-compose-dev.yml exec front_django /bin/sh -c "bin/docker/db-update.sh"

echo "== Loading dev fixtures (admin)"
sudo docker-compose -f bin/docker/docker-compose-dev.yml exec front_django /bin/sh -c "python manage.py loaddata bin/docker/fixtures-dev.yaml"

echo "== Generating docs"
sudo docker-compose -f bin/docker/docker-compose-dev.yml exec front_django /bin/sh -c "sphinx-build -E -b html ./docs ./.data/docs"
